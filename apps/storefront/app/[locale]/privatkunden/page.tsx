...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const t = await getTranslations({ locale: params.locale, namespace: "privatkunden" });
  const canonicalUrl = params.locale === "de" ? `${process.env.NEXT_PUBLIC_STOREFRONT_URL || "https://webshop.wsp-solarenergie.de"}/privatkunden` : `${process.env.NEXT_PUBLIC_STOREFRONT_URL || "https://webshop.wsp-solarenergie.de"}/${params.locale}/privatkunden`;
  return { 
    title: t("meta_title"), 
    ...
    alternates: {
      ...
      languages: {
        de: `${process.env.NEXT_PUBLIC_STOREFRONT_URL || "https://webshop.wsp-solarenergie.de"}/privatkunden`,
        en: `${process.env.NEXT_PUBLIC_STOREFRONT_URL || "https://webshop.wsp-solarenergie.de"}/en/privatkunden`,
        es: `${process.env.NEXT_PUBLIC_STOREFRONT_URL || "https://webshop.wsp-solarenergie.de"}/es/privatkunden`,
      },
    },
  };
}

export default async function PrivatkundenPage({ params }: Props) {
  const t = await getTranslations({ locale: params.locale, namespace: "privatkunden" });
  const profiles = t.raw("profiles") as { label: string; description: string; fit: string[] }[];
  ...
  const trustItems = t.raw("trust_items") as string[];
      ...
      <section className="py-28 sm:py-36 border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-xs text-brand-muted mb-10">
            ...
            <span>/</span>
            <span className="text-brand-text">{t("eyebrow")}</span>
          </nav>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-xl mb-12">
            <p className="text-xs font-medium text-brand-accent uppercase tracking-widest mb-3">{t("profile_eyebrow")}</p>
            <h2 className="font-display text-3xl font-bold text-brand-text mb-3">{t("profile_h2")}</h2>
            <p className="text-brand-muted leading-relaxed">{t("profile_sub")}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {profiles.map((profile) => (
              <div key={profile.label} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 flex flex-col gap-5">
                <div>
                  <p className="text-xs font-medium text-brand-accent uppercase tracking-widest mb-3">{profile.label}</p>
                  <p className="text-sm text-brand-muted leading-relaxed">{profile.description}</p>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gray-50 rounded-2xl p-5 space-y-2">
                {trustItems.map((item) => (
                  <p key={item} className="flex items-center gap-2.5 text-sm text-brand-muted">
